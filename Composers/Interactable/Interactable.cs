...
public partial class Interactable : Node
{
	...
	private void OnBodyEntered(Node body)
	{
		...
		{
			EmitSignal(SignalName.InteractionStart, player);
		}
		else if (body is Settler settler)
		{
			EmitSignal(SignalName.InteractionStart, settler);
		}
	}
	private void OnBodyExited(Node body)
	{
		if (body is Player player)
		{
			EmitSignal(SignalName.InteractionEnd, player);
		} else if (body is Settler settler)
		{
			EmitSignal(SignalName.InteractionEnd, settler);
		}
	}   
}
